...
namespace {
	void OnEvent(Blam::DatumHandle player, const Event *event, const EventDefinition *definition) {
		FILE *myfile;
		myfile = fopen("event_dump.txt", "a");
		
		//Write player

		const Blam::Players::PlayerDatum *ply;
		ply = Blam::Players::GetPlayers().Get(player.Handle);
			if (ply) {
				fprintf(myfile, "uid: %" PRIu64 " | player: %s  | ", ply->Properties.Uid, Utils::String::ThinString(ply->Properties.DisplayName).c_str());
			} else {
				fprintf(myfile, "uid: null | player: null | ");
			}
		

		//Write event
		fprintf(myfile, "event: %s", GetEventName(event->NameStringId).c_str());
